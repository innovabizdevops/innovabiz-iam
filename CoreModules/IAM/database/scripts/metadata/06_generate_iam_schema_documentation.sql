...
SELECT '# Documentação do Esquema IAM - INNOVABIZ
';

SELECT '## Visão Geral
';

SELECT 'Este documento descreve a estrutura do banco de dados do módulo de Identidade e Gerenciamento de Acesso (IAM) da plataforma INNOVABIZ. O esquema IAM é responsável por gerenciar usuários, funções, permissões, autenticação, autorização e auditoria no sistema.
';
...
SELECT '| ' || 
       ...
       CASE WHEN idx.indisunique THEN 'SIM' ELSE 'NÃO' END || ' | ' ||
...
---
*Documentação gerada automaticamente pelo script `06_generate_iam_schema_documentation.sql`*';
