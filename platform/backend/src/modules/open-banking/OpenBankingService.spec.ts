...
describe('OpenBankingService', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({ providers: [OpenBankingService, PRISMA_MOCK_PROVIDER] }).compile();
        service = module.get<OpenBankingService>(OpenBankingService);
    });

    it('should be defined', () => { expect(service).toBeDefined(); });
    it('should return hello', () => { expect(service.getHello()).toContain('Open Banking'); });

    describe('domain methods', () => {
        it('should return APIs', () => { const r = service.getApis() as any; expect(r).toBeDefined(); });
        it('should return TPPs', () => { const r = service.getTpps() as any; expect(r).toBeDefined(); expect(r.totalTpps).toBeGreaterThan(0); });
    });
