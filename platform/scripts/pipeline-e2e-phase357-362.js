#!/usr/bin/env node
/**
 * ABSOLUTE FINAL TO 3500+ — Phases 357-362 (12 modules = ~50+ new → 3500+!)
 */
...
const PHASES = {
    357: {
        label: "Precious Metals & Commodities", modules: [
            { id: "precious-metals", title: "Precious Metals", accent: "#d97706", subs: ["gold-trade", "silver-trade", "platinum-trade", "palladium-trade", "refinery-ops", "bullion-vault", "etf-metal", "futures-metal", "assay-certify", "supply-chain-metal"] },
            { id: "commodity-trade", title: "Commodity Trading", accent: "#b45309", subs: ["grain-trade", "coffee-trade", "cocoa-trade", "sugar-trade", "cotton-trade", "rubber-trade", "timber-trade", "iron-ore", "copper-trade", "lithium-trade"] },
        ]
    },
    358: {
        label: "Space Tourism & Commercial", modules: [
            { id: "space-tourism", title: "Space Tourism", accent: "#4338ca", subs: ["booking-space", "training-astronaut", "launch-manifest", "capsule-prep", "flight-ops-space", "re-entry", "recovery-ops", "medical-clear", "liability-waiver", "experience-design-space"] },
            { id: "commercial-space", title: "Commercial Space", accent: "#3730a3", subs: ["payload-broker", "launch-vehicle", "satellite-deploy", "space-station", "debris-track", "spectrum-coord", "remote-sensing", "ground-station", "mission-control", "insurance-space"] },
        ]
    },
    359: {
        label: "Indigenous & Traditional Knowledge", modules: [
            { id: "indigenous-knowledge", title: "Indigenous Knowledge", accent: "#059669", subs: ["oral-history", "traditional-medicine", "craft-heritage", "land-rights", "language-preserve", "cultural-protocol", "benefit-sharing", "bioprospect", "sacred-site", "repatriation"] },
            { id: "traditional-practice", title: "Traditional Practice", accent: "#047857", subs: ["herbal-medicine", "acupuncture-mgmt", "ayurveda", "traditional-birth", "bone-setting", "cupping-therapy", "homeopathy", "naturopathy", "traditional-diet", "practice-certify"] },
        ]
    },
    360: {
        label: "Prison & Correctional Mgmt", modules: [
            { id: "correctional-mgmt", title: "Correctional Mgmt", accent: "#dc2626", subs: ["inmate-intake", "classification", "housing-assign", "visitation-mgmt", "commissary-ops", "grievance-process", "disciplinary-hear", "sentence-compute", "reentry-plan", "staff-sched-corr"] },
            { id: "probation-parole", title: "Probation & Parole", accent: "#b91c1c", subs: ["case-load", "supervision-plan", "check-in", "drug-test", "electronic-monitor", "community-service", "victim-notify", "risk-assess-prob", "violation-report", "discharge-plan"] },
        ]
    },
    361: {
        label: "Disaster & Emergency Mgmt", modules: [
            { id: "disaster-mgmt", title: "Disaster Mgmt", accent: "#ef4444", subs: ["hazard-map", "vulnerability-assess-dis", "preparedness-plan", "early-warning", "evacuation-plan", "shelter-mgmt", "damage-assess", "relief-distribute", "recovery-plan", "mitigation-proj"] },
            { id: "emergency-ops", title: "Emergency Ops", accent: "#dc2626", subs: ["eoc-activation", "incident-command", "resource-dispatch", "mutual-aid", "comm-interop", "public-alert", "volunteer-coord", "donation-mgmt-em", "after-action", "training-exercise"] },
        ]
    },
    362: {
        label: "Oceanography & Marine Science", modules: [
            { id: "ocean-research", title: "Ocean Research", accent: "#0891b2", subs: ["cruise-plan-ocean", "ctd-sampling", "buoy-network", "acoustic-survey", "rov-ops", "deep-sea", "coral-monitor", "plankton-study", "ocean-model", "data-archive-ocean"] },
            { id: "marine-conserve", title: "Marine Conservation", accent: "#0e7490", subs: ["mpa-mgmt", "species-monitor", "habitat-restore", "turtle-nest", "whale-track", "mangrove-plant", "seagrass-restore", "pollution-monitor", "fishing-sustain", "blue-carbon"] },
        ]
    },
};

function richPage(fn, t, m) { return `"use client";\nimport React from "react";\nimport { BarChart3,TrendingUp,Clock,CheckCircle2,Search,Filter,Download,Plus,MoreHorizontal,ArrowUpRight,ArrowDownRight } from "lucide-react";\n\nconst PAGE="${t}";\nconst KPIS=[{l:"Total ${t}",v:"2,847",d:"+12.3%",u:true},{l:"${t} Ativos",v:"1,923",d:"+5.1%",u:true},{l:"Pendentes",v:"124",d:"-8.4%",u:false},{l:"Score (%)",v:"94.2%",d:"+2.1%",u:true}];\nconst COLS=["ID","Nome","Categoria","Responsável","Data","Estado"];\nconst ROWS=[{id:"001",c:["${t}-001","Alpha","A","Ana","2026-02-20","Ativo"]},{id:"002",c:["${t}-002","Beta","B","João","2026-02-19","Em Revisão"]},{id:"003",c:["${t}-003","Gamma","A","Maria","2026-02-18","Concluído"]},{id:"004",c:["${t}-004","Delta","C","Pedro","2026-02-17","Pendente"]},{id:"005",c:["${t}-005","Epsilon","B","Carla","2026-02-16","Ativo"]}];\nconst CHART=[65,78,52,90,85,72,91,68,83,76,89,94];\nconst ACTIVITY=[{t:"Novo em ${t}",ts:"2min",a:"Sys"},{t:"Aprovação #247",ts:"15min",a:"Ana"},{t:"Relatório",ts:"1h",a:"Auto"},{t:"Update",ts:"3h",a:"João"}];\n\nexport default function ${fn}(){return(<div className="p-6 space-y-6 max-w-[1600px] mx-auto"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h1 className="text-2xl font-bold tracking-tight">{PAGE}</h1><p className="text-sm text-muted-foreground">${m} — iBOS</p></div><div className="flex gap-2"><button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm hover:bg-accent"><Download className="h-3.5 w-3.5"/> Exportar</button><button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-sm hover:bg-primary/90"><Plus className="h-3.5 w-3.5"/> Novo</button></div></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">{KPIS.map((k,i)=>(<div key={i} className="rounded-xl border bg-card p-4 hover:shadow-md transition-shadow"><div className="flex items-center justify-between"><p className="text-xs font-medium text-muted-foreground">{k.l}</p>{i===0?<BarChart3 className="h-4 w-4 text-muted-foreground"/>:i===1?<TrendingUp className="h-4 w-4 text-muted-foreground"/>:i===2?<Clock className="h-4 w-4 text-muted-foreground"/>:<CheckCircle2 className="h-4 w-4 text-muted-foreground"/>}</div><p className="text-2xl font-bold mt-2">{k.v}</p><div className="flex items-center gap-1 mt-1">{k.u?<ArrowUpRight className="h-3 w-3 text-emerald-500"/>:<ArrowDownRight className="h-3 w-3 text-red-500"/>}<span className={\`text-xs font-medium \${k.u?"text-emerald-500":"text-red-500"}\`}>{k.d}</span></div></div>))}</div><div className="grid grid-cols-1 lg:grid-cols-3 gap-4"><div className="lg:col-span-2 rounded-xl border bg-card p-4"><h3 className="text-sm font-semibold mb-4">Evolução</h3><div className="flex items-end gap-1.5 h-32">{CHART.map((v,i)=>(<div key={i} className="flex-1 rounded-t hover:opacity-80" style={{height:\`\${v}%\`,background:\`hsl(\${200+i*10},70%,50%)\`}}/>))}</div></div><div className="rounded-xl border bg-card p-4"><h3 className="text-sm font-semibold mb-3">Atividade</h3><div className="space-y-3">{ACTIVITY.map((a,i)=>(<div key={i} className="flex gap-3 items-start"><div className="h-2 w-2 rounded-full bg-primary mt-1.5 shrink-0"/><div><p className="text-xs">{a.t}</p><p className="text-[10px] text-muted-foreground">{a.a}·{a.ts}</p></div></div>))}</div></div></div><div className="rounded-xl border bg-card"><div className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b"><h3 className="text-sm font-semibold">Registos</h3><div className="flex gap-2"><div className="relative"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground"/><input placeholder="Pesquisar..." className="pl-8 pr-3 py-1.5 rounded-lg border text-sm w-48 bg-transparent"/></div><button className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border text-sm hover:bg-accent"><Filter className="h-3.5 w-3.5"/> Filtrar</button></div></div><div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b bg-muted/30">{COLS.map(c=>(<th key={c} className="text-left text-xs font-medium text-muted-foreground px-4 py-2.5">{c}</th>))}<th className="w-10"/></tr></thead><tbody>{ROWS.map(r=>(<tr key={r.id} className="border-b last:border-0 hover:bg-muted/20">{r.c.map((cell,j)=>(<td key={j} className="px-4 py-2.5 text-sm">{j===r.c.length-1?(<span className={\`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium \${cell==="Ativo"?"bg-emerald-500/10 text-emerald-500":cell==="Concluído"?"bg-blue-500/10 text-blue-500":cell==="Pendente"?"bg-amber-500/10 text-amber-500":"bg-purple-500/10 text-purple-500"}\`}>{cell}</span>):cell}</td>))}<td className="px-2"><button className="p-1 hover:bg-accent rounded"><MoreHorizontal className="h-4 w-4 text-muted-foreground"/></button></td></tr>))}</tbody></table></div><div className="p-3 border-t flex items-center justify-between text-xs text-muted-foreground"><span>1-5</span><div className="flex gap-1"><button className="px-2 py-1 rounded border hover:bg-accent">←</button><button className="px-2 py-1 rounded border bg-primary text-primary-foreground">1</button><button className="px-2 py-1 rounded border hover:bg-accent">→</button></div></div></div></div>);}\n`; }
...
const ICONS = ["Gem", "Rocket", "Feather", "Lock", "Siren", "Waves", "Coins", "Satellite", "Leaf", "Scale", "AlertTriangle", "Anchor"];
...
console.log("\n╔════════════════════════════════════════════╗");
console.log("║  🎯 MÓDULOS TOTAIS: " + ids.size);
console.log("║  Novos: " + n + " | Dupes: " + d + " | Páginas: " + p);
